...
export default function CountryBlockPanel() {
    ...
    useEffect(() => {
        ...
        debounceRef.current = window.setTimeout(async () => {
            try {
                const js = await apiPostJson<{ ok: boolean; messages?: string[]; blocked?: string[] }>(
                    "/country-block-rules:validate",
                    { raw }
                );
                setValid(!!js.ok);
                setMessages(Array.isArray(js.messages) ? js.messages : []);
                setBlocked(Array.isArray(js.blocked) ? js.blocked : []);
            } catch (e: any) {
                setValid(false);
    ...
    const doSave = useCallback(async () => {
        ...
        setError(null);
        try {
            const js = await apiPutJson<{ ok: boolean; etag?: string; blocked?: string[] }>(
                "/country-block-rules",
                { raw },
                ...
                <div className="flex items-center justify-between text-xs text-neutral-500">
                    ...
                    <div className="flex items-center gap-2 max-w-[60%] overflow-hidden">
                        ...
                        {blocked.slice(0, 12).map((code) => (
                            <span key={code} className="px-2 py-0.5 bg-neutral-100 rounded">
                                {code}
                            </span>
                        ))}
                    </div>
