...
export default function BotDefensePanel() {
    ...
    useEffect(() => {
        ...
        debounceRef.current = window.setTimeout(async () => {
            try {
                const js = await apiPostJson<{ ok: boolean; messages?: string[]; enabled?: boolean; mode?: string; path_prefixes?: string[] }>(
    ...
    const doSave = useCallback(async () => {
        ...
        setError(null);
        try {
            const js = await apiPutJson<{ ok: boolean; etag?: string; enabled?: boolean; mode?: string; path_prefixes?: string[] }>(
                "/bot-defense-rules",
                { raw },
            ...
            <div className="grid gap-3">
                <div className="flex items-center justify-between gap-2">
                    <div className="text-sm text-neutral-500">
                        Manage suspicious-UA challenge settings in JSON (`mode`, `path_prefixes`, `exempt_cidrs`, cookie/TTL/status).
                    </div>
                    <div className="flex items-center gap-2">
                        <button
                            type="button"
