...
func TestComposeInitialRuleFiles_WithCRS(t *testing.T) {
	...
	got, err := composeInitialRuleFiles("rules/mamotama.conf", true, setup, rulesDir, filepath.Join(dir, "crs-disabled.conf"))
	if err != nil {
...
func TestComposeInitialRuleFiles_WithoutCRS(t *testing.T) {
	got, err := composeInitialRuleFiles("rules/mamotama.conf", false, "", "", filepath.Join(t.TempDir(), "crs-disabled.conf"))
	if err != nil {
		t.Fatalf("composeInitialRuleFiles() error = %v", err)
	}
	want := []string{"rules/mamotama.conf"}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("composeInitialRuleFiles() = %v, want %v", got, want)
	}
}
...
func TestComposeInitialRuleFiles_MissingCRS(t *testing.T) {
	_, err := composeInitialRuleFiles("rules/mamotama.conf", true, "missing-setup.conf", "missing-dir", filepath.Join(t.TempDir(), "crs-disabled.conf"))
	if err == nil {
...
func TestComposeInitialRuleFiles_WithCRSDisabledFile(t *testing.T) {
	...
	got, err := composeInitialRuleFiles("rules/mamotama.conf", true, setup, rulesDir, disabledFile)
	if err != nil {
		t.Fatalf("composeInitialRuleFiles() error = %v", err)
	}
