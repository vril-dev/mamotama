...
func TestLogsStatsAggregatesWAFBlocks(t *testing.T) {
	...
	c.Request = httptest.NewRequest(http.MethodGet, "/mamotama-api/logs/stats?scan=100&hours=6", nil)
...
func TestLogsStatsMissingLogReturnsEmptyPayload(t *testing.T) {
	...
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodGet, "/mamotama-api/logs/stats", nil)
