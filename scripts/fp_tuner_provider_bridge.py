...
def build_rule_line(path: str, rule_id: int, variable: str) -> str:
    digest = hashlib.sha1(f"{rule_id}|{path}|{variable}".encode("utf-8")).hexdigest()
    generated_id = 190000 + (int(digest[:6], 16) % 9000)
    return (
        f'SecRule REQUEST_URI "@beginsWith {path}" '
        f'"id:{generated_id},phase:1,pass,nolog,ctl:ruleRemoveTargetById={rule_id};{variable},msg:\'mamotama fp_tuner scoped exclusion\'"'
    )
...
def fill_proposal_defaults(proposal: Dict[str, Any], req_obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    target_path = str(req_obj.get("target_path") or proposal.get("target_path") or "rules/mamotama.conf").strip()
    if not target_path:
    ...
    if not str(out.get("summary", "")).strip():
        out["summary"] = f"Exclude {variable} from rule {rule_id} under path prefix {path}."
    ...
    if not str(out.get("rule_line", "")).strip():
        out["rule_line"] = build_rule_line(path, rule_id, variable)
